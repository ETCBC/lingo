...
class getHeads:
    ...
    *   `head` = phrase head to phrase mapping according to phrase type and word
        independency tests;
        also performed for phrase atoms by removing `phrase_atom` independency
        tests. To be exported as an edge feature from a head to a phrase.
    *   `obj_prep` = object of preposition mapping to the preposition according
        to word independency tests; to be exported as an edge feature from an
        object to its preposition.
    *   `nhead` = nominal head (`subs`, `adjv`, etc.) within any phrase, including
        prepositional phrases, with independency tests to be exported as an edge
        feature from an `nhead` to a phrase.

    Each template is stored in a search dictionary which contains
    the template and head/obj indices to the result.

    The resulting feature dictionaries are stored as members `head`,
    `obj_prep`, and `nhead`.
    """
